...
| `probation` | `map \| null` | ✅ | `{ startDate: string, endDate: string, checkIns: Array<{ id: string, dayOffset: 30 \| 60 \| 90, scheduledDate: string, completedDate: string \| null, notes: string \| null, rating: 1\|2\|3\|4\|5 \| null, completedBy: string \| null }>, probationOutcome: 'pending' \| 'passed' \| 'extended' \| 'terminated' } \| null` (P3-E27-D2). `null` until first activation. `startDate`/`endDate`/`scheduledDate`/`completedDate` are plain `YYYY-MM-DD` strings, not Timestamps, matching `jobs.scheduledDate`. Initialized by the `onStaffStatusActivated` Firestore trigger the moment `status` transitions `!= 'active' -> 'active'` (guarded on `probation` still being `null`, so an `active -> inactive -> active` reactivation never overwrites an in-progress record) — 3 check-ins generated at Day 30/60/90. Not in the self-service `hasOnly()` list — same trust model as `onboardingChecklist`/`backgroundCheck`: check-in completion and outcome changes are written directly by an admin session via `StaffDetailPanel` (`isAdmin()` already grants unrestricted write access), picked up by `onStaffUpdatedTrigger` for `auditLog`. `probationOutcome: 'terminated'` also sets top-level `status: 'inactive'` — nothing else; P3-E27-D3's future `onStaffDeactivated` trigger reacts to that transition for the actual offboarding checklist. `'extended'` appends 3 more check-ins 90 days out and resets `probationOutcome` back to `'pending'`. `onProbationCheckInDue` (daily scheduled fn) emails + SMS's Lauren when a check-in's `scheduledDate` is today and still incomplete. |
| `offboarding` | `map \| null` | ✅ | `{ deactivatedAt: Timestamp \| null, checklist: { authRevoked: boolean, keysReturned: boolean, accessCodesChanged: boolean, finalPayCalculated: boolean, recordArchived: boolean }, finalNotes: string \| null, departureReason: 'voluntary' \| 'performance' \| 'seasonal' \| 'other' \| null } \| null` (P3-E27-D3). `null` until first deactivation; **fully overwritten** (not guarded like `probation`'s "already initialized" check) on every `!= 'inactive' -> 'inactive'` transition, since each deactivation is a fresh departure event — a stale checklist from a *previous* departure must not persist through a reactivate → re-deactivate cycle. Initialized by the `onStaffDeactivated` Firestore trigger, which also calls `auth.updateUser(disabled: true)`, `auth.setCustomUserClaims({role: 'inactive'})`, and `auth.revokeRefreshTokens()` — the third call is beyond the original narrative spec's literal list but necessary: `disabled: true` alone only blocks *new* sign-ins, an already-issued ID token stays valid until its own expiry otherwise. `checklist.authRevoked` is trigger-set only, never admin-editable; the other 3 checklist booleans, `finalNotes`, and `departureReason` are admin-editable via `StaffDetailPanel` (direct write, `isAdmin()` already unrestricted, same trust model as `onboardingChecklist`/`probation`), picked up by `onStaffUpdatedTrigger` for `auditLog`. `checklist.recordArchived` is schema-only — always `false`, no admin toggle, no archival job; that's P3-E15's (Data Retention & PIPEDA Right-to-Erasure) job, not built here. Reactivation goes through the admin-gated `reactivateStaff` **callable** (not a direct client write — restoring Auth access requires real Admin SDK calls), which re-enables the account, restores the role-mapped custom claim, and resets `status: 'active'`, `probation: null`, `offboarding: null` (reactivation is treated as a new employment period). |
